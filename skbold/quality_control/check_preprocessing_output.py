...
def check_mc_output(directory, sub_id='sub', cutoff_spike=1,
    ...
    split : float
        Size of train set (in proportion, i.e. <1)
    Returns
    -------
    df : Dataframe
    ...
    df['Rec. conservative'] = np.all(np.c_[df['Max-min'] < 1.5,
                                           df['Nr. spikes'] == 0],
                                     axis=1).astype(int)
...
def check_nifti_header(directory, sub_id='sub', task_id='func',
    ...
    calc_zeros : bool
        Whether to calculate the number of voxels which are zero
        (e.g. outside mask);
        Takes significantly longer if true, because has to read in func file.
    output_dir : str
    ...
    for sub_dir in sub_dirs:
        ...
        for task_dir in task_dirs:
            ...
            df_tmp = {'Subject': op.basename(sub_dir), 'Task': task_name,
                      'TR': tr, 'zero_voxels': zero_voxels,
                      'n_slices': n_slices, 'dim1': dim1, 'dim2': dim2,
