...
def make_counterbalanced_split(file_name, variable_name=None, split=0.5,
    ...
    if isinstance(file_name, pd.DataFrame):
        ...
        raise ValueError('File_name should be either a filename of a '
                         ' c/tsv or a dataframe itself.')
    ...
    if method == 'Kolmogorov-Smirnov':
        ...
        raise ValueError("Use either 'Kolmorov-Smirnov' or "
                         "'Mann-Whitney-U' test!")

    sub_ids = ['Subject', 'Sub', 'sub_id', 'Subject_id']
    sub_col = [col for col in df.columns if col in sub_ids]
    if sub_col:
    ...
    for i in range(iterations):
        ...
        p_val = np.array([opt_func(train[col], test[col])
                          for col in df.columns]).mean()
    ...
    test_df = df.iloc[[item for item in range(n_inst)
                       if item not in set(results[p_max][0])]]
    ...
    if plot:
        ...
        for col in train_df.columns:
            ...
            plt.legend()
            plt.title('Train-test split, variable=%s, split=%f, iter=%i,
                       method=%s' % (col, split, iterations, method))
