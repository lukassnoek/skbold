...
def parse_roi_labels(atlas_type='Talairach', lateralized=False, debug=False):
    ...
    if not lateralized and atlas_type == 'HarvardOxford-Subcortical':
        info_dict = {'Cerebral_White_Matter': (0, (0, 0, 0)),
                       'Cerebral_Cortex': (1, (0, 0, 0)),
                       'Lateral_Ventricle': (2, (0, 0, 0)),
                       'Thalamus': (3, (0, 0, 0)),
                       'Caudate': (4, (0, 0, 0)),
                       'Putamen': (5, (0, 0, 0)),
                       'Pallidum': (6, (0, 0, 0)),
    ...
    rois = [r.replace('(', '').replace(')', '') for r in rois]
    rois = [r.replace(' ', '_').replace(',', '') for r in rois]
    rois = [r.replace("'", '') for r in rois]
